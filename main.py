...
def check_login1(user_name, user_password, friends_names):  # check input data and existing user
    ...
    if user_name == "" and user_password == "":
        ...
        for i in friends_names:
            if user_name + " " + user_password in i:
                ok = 1
    return ok
...
# check functions
def check_file_name(file_name, cur_month, cur_year):  # suppose to get: file - "Export_12_2020.xls" (str), cur_month (int), cur_year (int)
    ...
    for i in file_name:
        if i == "_" and file_name[file_name.index("_") + 3] == "_":  # months: 10, 11, 12
            month = file_name[file_name.index("_") + 1:file_name.index("_") + 3]
            year = file_name[file_name.index("_") + 4:file_name.index("_") + 8]
            break
        elif i == "_" and file_name[file_name.index("_") + 2] == "_":  # months: 1-9 or 01-09
            month = file_name[file_name.index("_") + 1:file_name.index("_") + 2]
            year = file_name[file_name.index("_") + 3:file_name.index("_") + 7]
            break
    ...
    if error == 1:
        ...
        window = sg.Window(title=design[7][0], layout=error_layout, size=design[7][2], modal=design[7][3], element_justification=design[7][4])
...
def sorting_layout(text, categories, empty_categories, cats, dates, names, charges):
    design = [[":אנחנו לא מכירים את ההוצאה הבאה", "", "Arial 14", "black", "white", (30, 3), 5, "c", True, ""],  # sort layout headline 1
              [text, "", "Arial 12", "red", "white", (30, 3), 5, "c", True, ""],  # sort layout headline 2
              ["?לאיזו קטגוריה שייכת ההוצאה", "", "Arial 14", "black", "white", (30, 3), 5, "c", True, ""],  # sort layout headline 3
              ["button_text", "", "Arial 12", ["black", "blue"], (12, 2), 1, True, False, "בחר/י קטגוריה זו במידה והיא מתאימה להוצאה המדוברת"],  # sort layout buttons - categories
              ...
              ["column_layout", "-col2-", "white", (800, 800), 1, "c", "c", True, True, True],  # export layout
              ["PayOff", "layout", (1200, 800), True, "c"],  # window
              [(500, 1), (960, 540), (970, 550), "graph", "black", 10, True, ""]]
    ...
    export_layout = [[sg.Text(":חיוב", key=design[4][1], font=design[4][2], text_color=design[4][3], background_color=design[4][4], size=design[4][5], pad=design[4][6], justification=design[4][7], visible=design[4][8], tooltip=design[4][9]),
                     ...
                     [sg.Graph(design[9][0], design[9][1], design[9][2], key=design[9][3], background_color=design[9][4], pad=design[9][5], visible=design[9][6], tooltip=design[9][7])]]
...
def delete_month(window, connection, exports_to, month_list, empty_month_list, display_keys, analysis_exports_or_flow_exports, button_list):  # for analysis and flow
        ...
        for i in exports_to:
            if i in month_list:
                ...
                for j in button_list:
                    if i[0] + "." + i[1] == j[1][0:len(j[1]) - end] or j[1][0:len(j[1]) - end] == int(empty_month_list.index(i)):  # TODO: check exception
    ...
    for i in empty_month_list:  # updates empty month list
...
def insert_current(connection, analysis_exports, empty_analysis_exports, flow_exports, empty_flow_exports, current, cur_day, cur_month, cur_year):  # insert for cur date, to current without sort
    ...
    if not values['value'] is None or len(current) == 0:
        for i in current:
...
def calculate_all_months(connection, analysis_exports, empty_analysis_exports, flow_exports, empty_flow_exports, current, cur_day, cur_month, cur_year):
    ...
    month_current = 0.0
    analysis_exports.reverse()  # to start from the oldest month , TODO: check why months order is changing
    a = 1
    for i in analysis_exports:
   ...
   #         for j in current:
    #            if j[1] == i[0] and j[2] == i[1]:
     #               i[16] = j[3]
        ...
        a += 1
    analysis_exports.reverse()  # bring back to right order - most recent to oldest month , TODO: check why months order is changing
...
def update_all_months(month_list, empty_month_list, display_keys, analysis_exports_or_flow_exports):
    ...
    for i in month_list:
        ...
        if i in empty_month_list:
            ...
            for j in display_keys:
                window[key + j + page1].update(value=i[display_keys.index(j)])  # TODO: check exception
    display_keys.reverse()
...
settings_design = [["הקטגוריות שלי", "home_title", "Arial 16", "black", "white", (15, 2), 10, "c", True, ""],
                   ...
                   ["לא קיים מידע זמין", "s_no_data", "Arial 10", "black", "white", (15, 1), 1, "r", True, ""],  # no data
...
for i in categories:
        ...
        if len(i) > 2:
            ...
            column.append([sg.Text(settings_design[9][0], key=i[0] + "." + settings_design[9][1], font=settings_design[9][2], text_color=settings_design[9][3], background_color=settings_design[9][4], size=settings_design[9][5], pad=settings_design[9][6], justification=settings_design[9][7], visible=settings_design[9][8], tooltip=settings_design[9][9])])
...
while True:
    ...
    # analysis options
        ...
        file_name = values['-IN-']  # string of full address of the export file
        ...
        files = 1
        [error, year, month] = check_file_name(file_name, cur_month, cur_year)
        ...
        if abroad_expn == 1:  # there are charges from abroad
        ...
        for i in names:
            type_name = type(i)
            type_charge = type(charges.iloc[a])
            type_date = type(dates.iloc[a])
            if not type(i) == str or not type(dates.iloc[a]) == str or not type(charges.iloc[a]) == int and not type(
                    charges.iloc[a]) == float:  # make sure data from file is ok
                data_error = 1
                break
            if i in skip_cash:
                continue
        ...
        for i in names:
            ...
            if sort == 0:  # if not in categories
                ...
                b = 0
                for j in categories:
                    if b + 1 == choice3:  # finds the chosen category from user choice, make "no data" un-visible and make button and text visible
                        if len(j) == 2:
                        ...
                        window[empty_categories[b][0] + "." + str(len(empty_categories[b])) + ".text"].update(
                            visible=True)
                        window[empty_categories[b][0] + "." + str(len(empty_categories[b])) + ".button"].update(
                            button_color=(s_buttons[7], s_buttons[8][0]))
                        window[empty_categories[b][0] + "." + str(len(empty_categories[b])) + ".button"].update(
                            disabled=False)
                        j.append(i)
                        ...
                        if j == 'cash':
                            month_expenses[14] = month_expenses[14] + charges.iloc[a]
                        try:
                            cursor.execute("INSERT INTO " + j[0] + " VALUES (?)", [
                                i])  # with no attention to doubles currently - between more than one category
                            connection.commit()
                        except:
                            general_error('שגיאה! קיימת בעיה עם מס"ד הנתונים')
                        break
        ...
        if a == len(names):  # make sure all charges were sorted
            ...
            for i in analysis_keys:
                if i == ".button":
                    window[empty_place1[empty_analysis_exports_place] + i + ".export"].update(
                        button_color=(a_buttons[7], a_buttons[8][0]))
                    window[empty_place1[empty_analysis_exports_place] + i + ".export"].update(disabled=False)
                    analysis_buttons.append(
                        [a_buttons[2], empty_place1[empty_analysis_exports_place] + i + ".export", buttonstate,
                         a_buttons[7], a_buttons[8]])
                    continue
                elif c == 15:
                    window[empty_place1[empty_analysis_exports_place] + i + ".export"].update(value=key)
                else:
                    window[empty_place1[empty_analysis_exports_place] + i + ".export"].update(
                        value=str(month_expenses[c]))
            ...
            if answer == "Yes":  # if this export has already been opened in the past, data will be replaced
                ...
                analysis_exports.append(month_expenses)
                analysis_exports_list.append(file_name)
